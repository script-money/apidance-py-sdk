...
from apidance.exceptions import ApiDanceError
...
def example_twitter_client():
    ...
    try:
        ...
        tweets = client.get_user_tweets(user_id="1846659274107404288", count=20)
        print(f"\nFound {len(tweets)} tweets:")
        print(tweets[0])

        # Example: Get tweet
        result = client.tweet_result_by_rest_id(tweet_id="1846659274107404288")
        print(result)

        # Example: Favorite tweet
        client.favorite_tweet("1895580962005917946")

        # Example: Reply to tweet
        client.create_tweet(
            text="You are geniuses! Are you? @OverlordBot_",
            reply_to_tweet_id="18771899856206399232345",
        )
        ...
        following = client.get_followers_you_know(
            user_id="1880358740152709121", count=30
        )
    ...
    except ApiDanceError as e:
        print(f"Catch error in example: {e.__class__.__name__} - {e}")
